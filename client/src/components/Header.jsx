...
const Header = () => {
    ...
    return (
        <header className="flex justify-between items-center px-6 py-4 bg-gradient-to-r from-green-500 to-green-700 shadow-lg sticky top-0 z-50">
            ...
            <Link 
                to="/" 
                className="text-white text-3xl font-bold hover:text-yellow-300 transition-colors"
            >
                AgriꞰart🌿
            </Link>
            ...
            <div className="hidden md:flex items-center space-x-6">
                {["Home", "About", "Services", "Contact", "Blog"].map((item) => (
                    <Link
                        ...
                        className="text-white hover:text-yellow-300 transition-all text-lg px-4 py-2 rounded-lg"
                    >
                        {item}
                    </Link>
                ))}
            ...
            <div className="flex items-center gap-4">
               {/* Cart Button */}
<button 
    className="relative w-12 h-12 bg-white rounded-full flex justify-center items-center shadow-md hover:scale-105 transition-all"
...
>
    ...
    {totalQuantity > 0 && (
        <span className="absolute -top-1 -right-1 bg-red-600 text-white text-xs font-semibold w-5 h-5 rounded-full flex justify-center items-center">
            {totalQuantity} {/* ✅ Displays the total number of items */}
        </span>
    )}
</button>
                ...
                {userData ? (
                    <div ref={dropdownRef} className="relative">
                        ...
                        {showDropdown && (
                            <div className="absolute top-12 right-0 bg-white shadow-xl rounded-lg py-2 w-44 text-black z-10">
                                <ul className="text-sm">
                                {!userData.isAccountVerified && (
    <li 
        onClick={() => navigate("/email-verify")} 
        className="px-4 py-2 text-green-600 font-semibold flex items-center gap-2 cursor-pointer hover:bg-gray-200"
    >
        <FiCheckCircle /> Verify Account
                    ...
                    <div className="flex gap-4">
                        <Link to="/login" className="bg-yellow-400 text-green-800 px-4 py-2 rounded-full shadow-md hover:bg-yellow-300 transition-all">
                            Log In
                        </Link>
                        <Link to="/signup" className="bg-yellow-500 text-green-800 px-4 py-2 rounded-full shadow-md hover:bg-yellow-400 transition-all">
                            Sign Up
                        </Link>
                    </div>
                )}
            ...
            {mobileMenuOpen && (
                <div className="absolute top-16 left-0 w-full bg-green-700 text-white py-4 shadow-md md:hidden">
                    {["Home", "About", "Services", "Contact", "Blog"].map((item) => (
                        <Link
                            key={item}
                            to={`/${item.toLowerCase().replace(" ", "-")}`}
                            className="block px-6 py-3 text-lg hover:bg-green-600 transition"
