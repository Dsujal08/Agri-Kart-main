...
const Detail = () => {
  ...
  useEffect(() => {
    if (!detail) {
      ...
      setSelectedImage(detail.image); // Set the main image initially
    }
  }, [slug, navigate, detail]);
  ...
  const handleAddToCart = () => {
    ...
    if (totalQuantity > detail.stock) {
      alert(`Only ${detail.stock} items available in stock!`);
      return;
    }
    ...
    alert(`${quantity} ${detail.name}(s) added to cart!`);
  };
